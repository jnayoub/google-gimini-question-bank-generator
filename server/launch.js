...
import { callGeminiPromptExisting } from "./endpoints/gimini-json-questions-answers-from-existing-questions.js";
...
app.get("/existing", (req, res) => {
    res.sendFile(
      path.join(
        ...
        "client",
        "pages",
        "google_ai_gimini_generate_questions_from_existing.html"
      )
    );
  });
...
app.post("/gemini-prompt-existing-questions", async (req, res) => {
    const { prompt, numberOfQuestions } = req.body;
    if (!prompt) {
      return res.status(400).send({ error: "No prompt provided" });
    }
    try {
      const text = await callGeminiPromptExisting(prompt, numberOfQuestions);
      res.json({ response: text });
    } catch (error) {
      console.error("Error generating content:", error);
      res.status(500).send({ error: "Failed to generate content" });
